...
export default function ThemeSelect({ }) {
  ...
  return (
    <div className="relative">
      <Listbox value={darkTheme} onChange={setDarkTheme}>
        <Listbox.Label className="sr-only">Theme</Listbox.Label>
        <Listbox.Button type="button" className="border dark:border-neutral-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-500 rounded-full p-1">
        ...
        >
          <Listbox.Option value={false} as={Fragment}>
            {({ active, selected }) => (
              ...
              >
                <SunIcon className="h-6 w-6" selected={darkTheme == false} />
                Light
              </li>
            )}
          </Listbox.Option>
          <Listbox.Option value={true} as={Fragment}>
            {({ active, selected }) => (
              <li
                className={clsx(
                  'py-1 px-2 gap-1 flex items-center cursor-pointer transition-all duration-200',
              ...
              >
                <MoonIcon className="h-6 w-6" selected={darkTheme == true} />
                Dark
              </li>
            )}
          </Listbox.Option>
