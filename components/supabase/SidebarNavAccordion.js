...
export default function SidebarNavAccordion({ name, routeName, icon, children, className }) {
  ...
  if (!cek) {
    return (
      <button
        className={`w-full flex justify-between px-2.5 py-1.5 items-center text-sm font-medium dark:text-white hover:bg-neutral-100 dark:bg-neutral-900 dark:hover:bg-neutral-800 rounded transition-all ${className}`}
      >
        ...
        {({ open }) => (
          <>
            ...
            {/* <Transition
              ...
              > */}
            <Disclosure.Panel className='px-2.5 text-sm flex flex-col space-y-1'>{children}</Disclosure.Panel>
            {/* </Transition> */}
          </>
        )}
      </Disclosure>
