...
export default function SidebarNavAccordion({ name, routeName, icon, children, className }) {
  ...
  if (!cek) {
    return (
      <button
        className={`flex w-full items-center justify-between rounded px-2.5 py-1.5 text-sm font-medium transition-all hover:bg-neutral-100 dark:bg-neutral-900 dark:text-white dark:hover:bg-neutral-800 ${className}`}
      >
        ...
        {({ open }) => (
          <>
            ...
            {/* <Transition
              ...
              > */}
            <Disclosure.Panel className='flex flex-col space-y-1 px-2.5 text-sm'>{children}</Disclosure.Panel>
            {/* </Transition> */}
          </>
        )}
      </Disclosure>
