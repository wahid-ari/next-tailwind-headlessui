      ...
      <nav className="py-2 overflow-x-auto scrollbar-thin scrollbar-thumb-gray-200 dark:scrollbar-thumb-neutral-700 scrollbar-thumb-rounded" aria-label="Breadcrumb">
        ...
        <ol className="flex items-center space-x-1 last:pr-4">
          <li className="flex items-center">
            ...
            <Link href="/" passHref className="ml-1 text-sm font-medium text-gray-500 hover:text-gray-900 dark:text-gray-300 dark:hover:text-white transition-all">
              Home
            </Link>
          </li>
          {paths.map((item, index) => {
            ...
            return (
              <li key={index} className="flex items-center">
                <ChevronRightIcon className="text-gray-500 dark:text-gray-300 w-4 h-4" />
                <Link href={breadcrumb[index]} passHref className="ml-1 text-sm font-medium text-gray-500 hover:text-gray-900 dark:text-gray-300 dark:hover:text-white transition-all">
                  {capitalizeFirstLetter(item)}
                </Link>
              </li>
            )
