...
export default function SidebarNavAccordion({ name, routeName, icon, children, className }) {
  ...
  if (!cek) {
    return (
      <button
        className={`flex w-full items-center justify-between rounded px-2 py-1.5 text-sm font-medium transition-all hover:bg-gray-100 dark:bg-neutral-900 dark:text-white dark:hover:bg-neutral-800 ${className}`}
      >
