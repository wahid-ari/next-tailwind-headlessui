...
const IndeterminateCheckbox = forwardRef(({ indeterminate, ...rest }, ref) => {
  ...
  return (
    <>
      <input type="checkbox" ref={resolvedRef} {...rest} className="h-4 w-4 border-gray-400 dark:border-neutral-600 rounded text-blue-500 focus:ring-blue-500
...
export const ReactTableSelect = forwardRef(({ columns, data, className, bordered, onRowSelectStateChange, setSelectedOriginalRows }, ref) => {
              ...
              <tr key={i + 1} {...headerGroup.getHeaderGroupProps()} className="text-left border-b text-sm dark:border-neutral-800 font-medium bg-gray-50 dark:bg-[#202020]">
                {headerGroup.headers.map((column, i) => (
                  <th key={i + 1} {...column.getHeaderProps(column.getSortByToggleProps())}
                    className={`font-semibold p-3 ${bordered && "first:border-l-0 last:border-r-0 border-x dark:border-x-neutral-800"}`}>
...
ReactTableSelect.displayName = 'ReactTableSelect';
