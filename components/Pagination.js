...
export default function Pagination({ className, min, max, current = 1 }) {
  ...
  return (
    <div className={`flex rounded ${className && className}`}>
      <button onClick={Prev} className="h-8 border dark:border-neutral-700 border-r-0 px-2 rounded-l hover:bg-sky-500 group transition-all">
      ...
      <button onClick={Next} className="h-8 border dark:border-neutral-700 px-2 rounded-r hover:bg-sky-500 group transition-all">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true"
